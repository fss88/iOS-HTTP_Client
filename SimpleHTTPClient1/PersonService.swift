...
class ClaimService {
    ...
    func addClaim(pObj : Claim, complition: @escaping(String) -> Void ) {
        ...
        let task = URLSession.shared.uploadTask(with: request, from: jsonData) {
            ...
            if let resp = data {
                ...
                complition("Claim (\(pObj.title)) was successfully created.")
            } else if let respError = error {
                print("Server Error : \(respError)")
                complition("Claim (\(pObj.title))failed to be created.")
            }
        }
        task.resume()
    }
    
    func getAll( complition: @escaping([Claim]) -> Void ) {
        //var ClaimList = [Claim]()
